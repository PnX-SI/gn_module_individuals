...
-- NOTE : le template amont (contrib/test_indi/synthese.sql de gn_module_monitoring)
-- utilise une variable :module_code / :'module_code' prévue pour être substituée par
-- `psql -v module_code=...`. Mais `process_sql_files` exécute ce fichier via
-- SQLAlchemy `text()`, qui ne peut pas lier un paramètre à l'intérieur d'un
-- identifiant (nom de vue) : ça échoue avec une erreur de syntaxe sur `:`.
-- Le module_code est donc codé en dur ci-dessous pour CMR_BOUQUETIN.

DROP VIEW IF EXISTS gn_monitoring.v_synthese_cmr_bouquetin;
CREATE VIEW gn_monitoring.v_synthese_cmr_bouquetin AS
...
WITH source AS (
	...
	WHERE name_source = CONCAT('MONITORING_', UPPER('CMR_BOUQUETIN'))
	LIMIT 1
...
SELECT
	...
	JOIN observers obs ON obs.id_base_visit = v.id_base_visit
 	...
 	LEFT JOIN LATERAL ref_geo.fct_get_altitude_intersection(s.geom_local) alt (altitude_min, altitude_max)
        ON TRUE
    WHERE m.module_code = 'CMR_BOUQUETIN'
    ;


SELECT * FROM gn_monitoring.v_synthese_cmr_bouquetin
